...
type TabType = 'playlist' | 'kids' | 'adults' | 'groups' | 'users';
...
const TABS: Tab[] = [
  ...
  { id: 'kids', label: 'Perfil Niños', icon: '🧒' },
  { id: 'adults', label: 'Perfil Adultos', icon: '👥' },
...
export default function AdminStatusPanel({ initialStatus }: { initialStatus: StatusResponse }) {
  ...
  return (
    <div className="mx-auto h-full max-w-5xl overflow-y-auto px-4 py-8 sm:px-8">
      <h1 className="mb-6 text-2xl font-bold tracking-tight text-white">Administración</h1>
      ...
      <div className="space-y-6">
        ...
        {/* Kids Tab */}
        {activeTab === 'kids' && <KidsConfig />}

        {/* Adults Tab */}
        {activeTab === 'adults' && <AdultsConfig />}

        {/* Groups Tab */}
        {activeTab === 'groups' && <CategoryGroupsConfig />}
